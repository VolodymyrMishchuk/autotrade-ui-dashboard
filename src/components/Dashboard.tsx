  ...
  const quickActions = [
    {
      ...
      icon: Users,
      action: () => setActiveSection("persons"),
      color: "bg-blue-500",
    },
    {
      title: "Add Account",
    ...
    },
    {
      ...
      icon: ArrowLeftRight,
      action: () => setActiveSection("transactions"),
      color: "bg-orange-500",
    },
  ];

  if (activeSection === "persons") {
    return <PersonsSection onBack={() => setActiveSection(null)} />;
  }

  if (activeSection === "accounts") {
    return <AccountsSection onBack={() => setActiveSection(null)} />;
  }

  if (activeSection === "sources") {
    return <SourcesSection onBack={() => setActiveSection(null)} />;
  }

  if (activeSection === "transactions") {
    return <TransactionsSection onBack={() => setActiveSection(null)} />;
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickActions.map((action, index) => (
              <Button
                key={index}
                variant="outline"
                className="h-auto p-4 flex flex-col items-center gap-3 hover:shadow-md transition-all duration-300"
